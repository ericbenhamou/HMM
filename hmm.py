...
class Hmm(object):
    ...
    def __alpha_recursion(self):
        ...
        # rescaled case
            ...
            for t in range(self.T - 1):
                self.alpha[t +
                           1, :] = np.multiply(self.b[t +
                                                      1, :], np.dot(self.a.T, self.alpha[t, :]))
    ...
    def __plot_states(self, data, T_max, title, prefix,
                      suffix, plot_type='plot'):
    ...
    def __llh_mixture(self, data):
        normals = [multivariate_normal(
            mean=self.mu0[i, :], cov=self.sigma0[i, :, :]) for i in range(self.K)]
        T = len(data)
    ...
    def print_loglikelihoods_table(self):
        ...
        print('\hline')
        print(
            '\diagbox[width=8em]{Iterations}{Dataset} & HMM Train & HMM Test \\\\ \hline')
        N = len(self.llh)
        for i in range(N):
            print(i + 1,
                  ' & ',
                  '{:.1f}'.format(self.llh[i,
                                           0]),
                  ' & ',
                  '{:.1f}'.format(self.llh[i,
                                           1]),
        ...
        print('\end{tabular}')
        print(
            '\caption{likelihood table for the HMM model in terms of EM iterations}')
        ...
        print('HMM & ', '{:.0f}'.format(
            self.llh[N - 1, 0]), ' & ', '{:.0f}'.format(self.llh[N - 1, 1]), '\\\\ \hline')
        print(
            'Mixture  & ', '{:.0f}'.format(
                self.__llh_mixture(
                    self.train_data)), ' & ', '{:.0f}'.format(
                self.__llh_mixture(
                    self.test_data)), '\\\\ \hline')
        print('\end{tabular}')
                ...
                for q in self.states:
                    (self.max_proba[t, q], self.max_index[t -
                                                          1, q]) = max((self.max_proba[t -
                                                                                       1, q0] +
                                                                        l_a[q0, q] +
                                                                        l_b[t, q], q0) for q0 in self.states)
