...
def compare_recusions():
    ...
    hmm2.EM()
    print(
        "log-scale version run in %.6s seconds " %
        (time.time() - start_time))

    print(
        'difference cond_proba ' %
        np.min(
            hmm.cond_proba -
            hmm2.cond_proba),
        np.max(
    ...
    print(
        ...
        np.min(
            hmm.joined_cond_proba -
            hmm2.joined_cond_proba),
        np.max(
            hmm.joined_cond_proba -
            hmm2.joined_cond_proba))
    return
...
hmm.plot_proba(
    100,
    'conditional proba with initial parameters on test',
    'q02',
    '1')
...
def plot_Viterbi_path(hmm):
    ...
    hmm.plot_most_likely_state(hmm.path, len(
        hmm.path), 'Most likely state (Viterbi) on train', 'q07', '1')
...
# Q10
print('\n****************** Q10 ******************')
hmm.plot_most_likely_state(np.argmax(
    hmm.cond_proba[:100, :], 1), 100, 'Most likely state (cond Proba) on Test', 'q10', '1')
hmm.plot_cluster(
    'Test',
    hmm.test_data,
    'Cond proba',
...
hmm.plot_most_likely_state(
    hmm.path,
    100,
    'Most likely state (Viterbi) on Test',
    'q11',
    '1')
hmm.plot_cluster('Test', hmm.test_data, 'viterbi', hmm.path, 'q11', '2')

# put a pause to make sure we get the figure set up when running in
# command line
try:
    plt.pause(5)
